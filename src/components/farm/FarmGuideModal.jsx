...
export default function FarmGuideModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 dark:bg-black/75 backdrop-blur-sm p-4 overflow-y-auto animate-in fade-in duration-200">
      <div
        className="relative w-full max-w-lg bg-white/95 dark:bg-[#0e1622]/95 border border-stone-200/90 dark:border-emerald-500/30 rounded-3xl p-5 sm:p-7 shadow-2xl select-none text-stone-800 dark:text-white my-8 overflow-hidden backdrop-blur-2xl transition-colors"
      ...
      >
        ...
        {/* Header */}
        <div className="flex items-center gap-3 border-b border-stone-200/80 dark:border-white/10 pb-4">
          <div className="w-12 h-12 rounded-2xl bg-emerald-500/15 dark:bg-emerald-500/20 border border-emerald-400/40 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xl shadow-sm shrink-0">
            📖
          </div>
          <div>
            <h2 className="text-xl font-black text-stone-900 dark:text-white font-sans">
        ...
        <div className="space-y-4 my-5">
          {steps.map((s) => (
            <div
              key={s.step}
              className="flex items-start gap-3.5 bg-stone-50/90 dark:bg-stone-800/40 border border-stone-200/80 dark:border-white/5 p-3.5 rounded-2xl shadow-sm"
            >
              <div className="w-10 h-10 rounded-xl bg-white dark:bg-stone-800 border border-stone-200 dark:border-white/10 flex items-center justify-center text-xl shrink-0 shadow-sm">
                {s.icon}
              </div>
              <div className="flex-1">
                ...
                <p className="text-xs text-stone-600 dark:text-stone-300/80 mt-1 leading-relaxed">
                  {s.desc}
                </p>
              </div>
            </div>
        ...
        {/* Pro Tip Box */}
        <div className="bg-amber-500/10 border border-amber-500/25 rounded-2xl p-3.5 flex items-start gap-3 text-xs text-amber-900 dark:text-amber-200/90 leading-relaxed mb-5 shadow-sm">
          <Sparkles size={18} className="text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
          <div>
            <strong className="text-amber-800 dark:text-amber-300 font-bold block mb-0.5">Mẹo của Nông Gia:</strong>
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-full h-11 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-emerald-950/20 cursor-pointer active:scale-98 transition-all"
        >
          <Check size={16} />
