    ...
    <div className="grid gap-3 grid-cols-4 w-full">
      {ratingButtons.map(({ label, value, keyHint, color }) => (
        <button
          ...
          className={`flex flex-col sm:flex-row items-center justify-center gap-1.5 rounded-full min-h-[44px] py-3 px-4 text-white font-mono font-bold text-xs sm:text-sm transition-all duration-150 cursor-pointer shadow-sm ${color}`}
          aria-label={`Đánh giá: ${label} (Phím ${keyHint})`}
        >
          <span>{label}</span>
          <span className="hidden sm:inline text-[10px] opacity-75 bg-black/20 px-2 py-0.5 rounded-full select-none">
            {keyHint}
          </span>
        </button>
      ))}
