...
export default function GardenScreen() {
    ...
    try {
      ...
      if (newPlants.length > 0) {
        setSelectedPlant((prev) => {
          if (prev) {
            const updated = newPlants.find(p => p.id === prev.id);
            return updated || newPlants[0];
          }
          return newPlants[0];
        });
      }
  ...
  const getGrassTileStyle = (row, col, isSelected) => {
    const isEven = (row + col) % 2 === 0;
    const baseColor = isEven
      ? 'bg-[#1b431c]/90 border border-[#2e6d30]/20'
      : 'bg-[#133215]/90 border border-[#214f24]/20';
    return isSelected
      ? `${baseColor} ring-4 ring-yellow-400 shadow-[0_0_20px_rgba(245,158,11,0.6)] z-10 scale-[1.02]`
      : `${baseColor} hover:brightness-110 hover:ring-2 hover:ring-white/10`;
  };
  ...
  // Active plant shown in Almanac: hovered plant takes priority, then selected plant, then the first plant
  const activePlant = hoveredPlant || selectedPlant || plants[0] || null;
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] w-full bg-[#0a0b10] rounded-3xl overflow-hidden p-4 sm:p-6 text-white border border-white/5 flex flex-col relative select-none font-sans">
      ...
      {/* 3. Split Layout: Grid Lawn vs Almanac Details */}
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0 items-start">
        
        {/* Left column: Lawn (8 cols) */}
        <div className="lg:col-span-8 flex flex-col h-full min-h-[500px]">
          <div className="flex-1 overflow-y-auto max-h-[calc(100vh-270px)] pr-2 rounded-3xl border border-white/5 shadow-2xl relative bg-gradient-to-b from-[#0d0e12] to-[#08090b]">
              ...
              {Array.from({ length: totalTiles }).map((_, tileIndex) => {
                ...
                if (!plant) {
                  return (
                    <div 
                      key={`empty-${tileIndex}`}
                      className={`relative flex flex-col items-center justify-center min-h-[140px] aspect-square rounded-xl transition-all duration-300 hover:brightness-105 group/tile ${getGrassTileStyle(row, col, false)}`}
                    >
                ...
                const { id, hanzi, stage, isOverdue } = plant;
                ...
                return (
                  <div
                    ...
                    className={`relative flex flex-col items-center justify-between py-3 min-h-[140px] aspect-square rounded-xl transition-all duration-300 cursor-pointer ${getGrassTileStyle(row, col, isSelected)}`}
        ...
        {/* Right column: Suburban Almanac Details (4 cols) */}
        <div className={`
          lg:col-span-4 bg-[#1c120c]/95 border-[#a16207] shadow-2xl flex flex-col gap-4 relative overflow-hidden select-none lg:sticky lg:top-4 lg:rounded-3xl lg:border-2 lg:translate-y-0 lg:flex
          ${mobileDrawerOpen 
            ? 'fixed bottom-0 left-0 right-0 z-40 rounded-t-3xl border-t-2 border-l-0 border-r-0 border-b-0 max-h-[75vh] overflow-y-auto animate-[slideUp_0.2s_ease-out_forwards]' 
            : 'hidden lg:flex'
          }
        `}
        style={{
          ...
          <div className="flex items-center justify-between border-b border-[#a16207]/45 pb-3">
            <div>
              <span className="text-[9px] font-sans font-black text-orange-400/70 uppercase tracking-widest block">
                SÁCH BÁCH KHOA
              </span>
              <h3 className="text-sm font-bold text-white font-serif mt-0.5">📖 HỒ SƠ THẢO MỘC</h3>
            </div>
            ...
            <button
              type="button"
              onClick={() => setMobileDrawerOpen(false)}
              className="lg:hidden flex items-center justify-center w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 border border-[#a16207]/30 text-[#f59e0b] cursor-pointer text-sm font-black transition-colors"
            >
              ✕
            </button>
          </div>
          ...
          {activePlant ? (
            <div className="flex flex-col gap-4">
              {/* Plant Large Preview Box */}
              <div className="bg-black/35 rounded-2xl border border-white/5 p-4 flex items-center justify-center h-44 relative overflow-hidden">
                ...
                {activePlant.stage === 'sprout' && (
                  <svg viewBox="0 0 60 60" className="w-24 h-24 animate-sway">
                    <ellipse cx="30" cy="50" rx="16" ry="4.5" fill="#3f1c06" opacity="0.5" />
                    <path d="M 30 50 Q 26 40 29 30" stroke={activePlant.isOverdue ? '#854d0e' : '#4ade80'} strokeWidth="3" fill="none" />
                    <path d="M 29 30 Q 21 26 19 32 Q 24 33 29 30" fill={activePlant.isOverdue ? '#7c2d12' : '#166534'} />
                    <path d="M 29 30 Q 37 25 40 31 Q 35 34 29 30" fill={activePlant.isOverdue ? '#b45309' : '#22c55e'} />
                  </svg>
                )}

                {activePlant.stage === 'sapling' && (
                  <svg viewBox="0 0 70 70" className="w-28 h-28 animate-sway">
                    <ellipse cx="35" cy="60" rx="20" ry="5.5" fill="#3f1c06" opacity="0.5" />
                    <path d="M 35 60 L 34 48 Q 32 36 35 24" stroke="#5c2e0b" strokeWidth="4.5" fill="none" strokeLinecap="round" />
                    <circle cx="27" cy="24" r="11" fill={activePlant.isOverdue ? '#7c2d12' : '#166534'} opacity="0.9" />
                    <circle cx="43" cy="26" r="11" fill={activePlant.isOverdue ? '#a16207' : '#15803d'} opacity="0.9" />
                    <circle cx="35" cy="15" r="14" fill={activePlant.isOverdue ? '#b45309' : '#22c55e'} opacity="0.95" />
                  </svg>
                )}

                {activePlant.stage === 'golden' && (
                  <svg viewBox="0 0 80 80" className="w-32 h-32 animate-sway animate-goldGlow">
                    <ellipse cx="40" cy="68" rx="24" ry="6.5" fill="#381a04" opacity="0.5" />
                    <path d="M 40 68 L 38 52 Q 36 36 40 22" stroke="#854d0e" strokeWidth="6" fill="none" strokeLinecap="round" />
                    <path d="M 38 46 Q 28 36 24 40" stroke="#854d0e" strokeWidth="3" fill="none" strokeLinecap="round" />
                    <path d="M 39 38 Q 49 28 53 32" stroke="#854d0e" strokeWidth="3" fill="none" strokeLinecap="round" />
                    <circle cx="26" cy="26" r="15" fill={activePlant.isOverdue ? '#a16207' : '#f59e0b'} opacity="0.9" />
                    <circle cx="54" cy="28" r="15" fill={activePlant.isOverdue ? '#78350f' : '#b45309'} opacity="0.9" />
                    <circle cx="40" cy="15" r="20" fill={activePlant.isOverdue ? '#d97706' : '#fbbf24'} opacity="0.95" />
                  </svg>
                )}
                
                {activePlant.isOverdue && (
                  <div className="absolute top-2 right-2 bg-rose-600 text-white rounded-full px-2 py-0.5 text-[9px] font-black border border-white/20 shadow">
                    🍂 Cần ôn tập
                  </div>
                )}
              </div>
              ...
              <div className="text-center">
                <span className="text-3xl font-black text-yellow-400 font-serif leading-none tracking-wider">{activePlant.hanzi}</span>
                <span className="text-sm text-orange-400 font-mono font-bold block mt-1">{activePlant.pinyin}</span>
              </div>

              {/* Meaning block */}
              <div className="bg-black/25 rounded-2xl p-3.5 border border-[#a16207]/20">
                <span className="text-[9px] font-sans font-bold text-orange-400/80 block mb-1">NGHĨA VIỆT:</span>
                <p className="text-xs font-semibold text-orange-100 leading-relaxed font-sans max-h-[90px] overflow-y-auto pr-1">
                  {activePlant.meaning}
                </p>
              </div>

              {/* Stats Grid */}
              <div className="grid grid-cols-2 gap-2 text-[10px] bg-black/15 p-3 rounded-2xl border border-white/5 font-sans">
                <div className="space-y-0.5">
                  <span className="text-white/40 block text-[8px]">GIAI ĐOẠN:</span>
                ...
                <div className="space-y-0.5">
                  <span className="text-white/40 block text-[8px]">GIÃN CÁCH SRS:</span>
                  <strong className="text-orange-200">{activePlant.interval} ngày</strong>
                </div>
                <div className="space-y-0.5 col-span-2 pt-1.5 border-t border-white/5">
                  <span className="text-white/40 block text-[8px]">TÌNH TRẠNG SỨC KHỎE:</span>
                  <strong className={activePlant.isOverdue ? 'text-rose-400 animate-pulse' : 'text-green-400'}>
                    {activePlant.isOverdue ? 'Khô héo (Cần ôn tập gấp để tưới)' : 'Tươi tốt'}
                  </strong>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-1">
                <button
                  type="button"
                  onClick={(e) => handleSpeak(e, activePlant.hanzi)}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-yellow-500/10 border border-yellow-500/35 hover:bg-yellow-500/25 text-yellow-400 text-xs font-bold cursor-pointer transition-all"
                >
                  <Volume2 size={13} />
                  <span>Đọc</span>
                </button>
                <Link
                  to={`/write?word=${encodeURIComponent(activePlant.hanzi)}`}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-orange-500/10 border border-orange-500/35 hover:bg-orange-500/25 text-orange-400 text-xs font-bold cursor-pointer transition-all"
                >
                  <PenTool size={13} />
                  <span>Tập viết</span>
                </Link>
            ...
            <div className="py-20 text-center text-xs text-orange-200/50 font-medium">
              🌱 Bãi cỏ chưa có cây nào.<br/>Hãy học từ vựng để gieo hạt!
            </div>
          )}
        </div>
